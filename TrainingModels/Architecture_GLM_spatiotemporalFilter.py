...
class LeNetConvPoolLayer_3dFilter(object):
    ...
    def __init__(self, rng, input, filter_shape, image_shape, padvalue,poolsize=(1, 1), outputType = 'l'):
        ...
        self.input = input
        fan_in = filter_shape[1]*filter_shape[3]*filter_shape[4]
        W_init = numpy.asarray(numpy.random.randn(1,filter_shape[1],1,filter_shape[3],filter_shape[4])*sqrt(2.0/fan_in),dtype=theano.config.floatX)
        self.W = theano.shared(value=W_init, name='W', borrow=True)

        b_values = -2*numpy.ones((1,), dtype=theano.config.floatX)
        ...
        if outputType == 'rl':
            ...
            self.output = self.lin_output +self.b
...
def SGD_training():
   ...
   """
    ...
    Layer1 = LeNetConvPoolLayer_3dFilter(rng, input = reshaped_input, filter_shape=(1, temporalFilterlength, 1, image_patch_size, image_patch_size), image_shape = (1, batch_size, 1, image_patch_size, image_patch_size),padvalue = stim_mean,poolsize=(1, 1), outputType = 'l')
    ...
    Layer2b_input = Layer1.output.reshape((batch_size,))
...
#                neurnum: i_n})
    ...
    while (not done_looping): # Keep looping until converged according to early stopping criteria
        ...
        if numpy.any(numpy.equal(epoch,numpy.arange(1,5000,5))):
            f = file('Theano_GLMs/GLM_fullspatiotemp_'+exp+'_'+cell_type+'_N'+str(n_neur)+'_undone.save', 'wb')
    ...
    # Save data
    f = file('Theano_GLMs/GLM_fullspatiotemp_'+exp+'_'+cell_type+'_N'+str(n_neur)+'.save', 'wb')
    for obj in [best_params + [test_score] + [test_losses] + [test_pred] + [test_actual] + [all_train_epochs] + [all_train_scores]  + [all_val_epochs] + [all_val_scores] + [all_test_epochs] + [all_test_scores] + [epoch] + [valInds] + [trainInds] + params]:
        cPickle.dump(obj, f, protocol=cPickle.HIGHEST_PROTOCOL)
    f.close()
